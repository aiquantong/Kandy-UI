 ...
 *  Camrea position
 */
@property(readonly) AVCaptureDevicePosition cameraPosition;
/**
...
/**
 *  This property is to define if the current routing should be changed when video call started
 */
@property(nonatomic,assign) BOOL shouldRouteToSpeakerOnVideoCall;
...
/**
 *  Sets specified video resolution for specified camera
 *
 *  @param cameraPosition   is indicating the physical position of an AVCaptureDevice's hardware on the system.
 *  @param videoResolution  is max video resolution.
 *  @param responseCallback is response callback for setting required max video resolution for specified capturing device.
 */
- (void)setCameraPosition:(AVCaptureDevicePosition)cameraPosition withVideoResolution:(NSString *)videoResolution responseCallback:(void (^)(NSError *error))responseCallback;

/**
 *  Switches between cameras with specified video resolution.
 *
 *  @param videoResolution  is AVCaptureSession preset value.
 *  @param responseCallback is reponse callback for switching between cameras.
 */
- (void)switchCameraWithVideoResolution:(NSString *)videoResolution responseCallback:(void (^)(NSError *error))responseCallback;
