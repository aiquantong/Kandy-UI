...
#import "KandyEventServiceNotificationDelegate.h"

@interface KandyEventService : NSObject

/**
 *  Register to event service notifications
 *
 *  @param notificationDelegate the delegate
 */
-(void) registerNotifications:(id<KandyEventServiceNotificationDelegate> _Nonnull)notificationDelegate;

/**
 *  Stop listening to event service notifications
 *
 *  @param notificationDelegate the delegate
 */
-(void) unregisterNotifications:(id<KandyEventServiceNotificationDelegate> _Nonnull)notificationDelegate;
 ...
 *  @param timestamp        The starting date to pull earlier or future events from. Pass nil value to pull very latest events starting from now
 ...
 *  @param maxCount         The maximum number of events to be pulled. If number of events on server is less than this number, all the events available will be pulled. If value passed is zero, all the events starting from the specified timestamp will be pulled
 ...
 */
-(void) pullHistoryEventsForDestination:(KandyRecord * _Nonnull)destination timestamp:(NSDate * _Nullable)timestamp moveForward:(BOOL)moveForward maxCount:(NSInteger)maxCount responseCallback:(void (^ _Nullable)(NSError * _Nullable error, NSInteger numberOfEventsReceived, BOOL hasMoreEventsToPull))responseCallback;
 ...
 *  @param responseCallback A callback that returns number of events pulled and flag indicating that more messages is awailabe to pull on success, or error if request was failed
 */
-(void) pullHistoryEventsWithTimestamp:(NSDate * _Nullable)timestamp moveForward:(BOOL)moveForward maxCount:(NSInteger)maxCount responseCallback:(void (^ _Nullable)(NSError * _Nullable error, NSSet<KandyRecord*> * _Nullable destinations, NSInteger numberOfEventsReceived, BOOL hasMoreEventsToPull))responseCallback;
